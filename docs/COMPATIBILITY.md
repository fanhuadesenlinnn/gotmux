...
| Basic mode/client entry commands | Implemented for `clock-mode`, `copy-mode`, `choose-buffer`, `choose-client`, `choose-tree`, `customize-mode`, and `find-window` empty CLI success, plus basic attached `display-panes` pane-number status feedback, basic attached `choose-tree` and `choose-buffer` summary feedback, and `display-panes`, `display-menu`, `display-popup`, `confirm-before`, `command-prompt`, and `suspend-client` no-current-client errors in the detached CLI subset |
...
| Basic options | Implemented for string-backed `set-option`, `set-window-option`, `show-options`, and `show-window-options`/`showw`, including global/local/server `-u`, string `-a`, basic `-o` set-once behavior, `show-options -A`, basic `show-options -H` hook output, basic `-s` server scope, basic `-t` session/window targets, `message-limit`, `status off` hiding the attached-client status line, and default `status-left`/`status-right` option values in the tested subset |
...
| Basic key bindings | Implemented for `bind-key`, `bind-key -N` notes, `unbind-key`, basic `unbind-key -a` table clearing, `list-keys`, missing-table `list-keys -T` errors, basic `list-keys -N` note output, `prefix`/`prefix2` dispatch, root table dispatch for simple keys, common arrow/navigation/function-key input sequences, default `prefix q` display-panes, and `send-prefix`/`send-prefix -2` |
...
| Basic pane geometry | Implemented for simple horizontal/vertical splits, nested splits, targeted `resize-pane -t`, basic `resize-pane -Z` zoom/unzoom with `window_flags`/`window_zoomed_flag`, targeted `select-layout -t` built-in layouts, basic next/previous layout cycling, basic same-window `swap-pane`, basic `rotate-window`, basic `break-pane`, and basic `join-pane`/`move-pane` in the tested subset |
| Basic terminal screen grid | Implemented for common printable output, cursor movement, clear line/screen, insertion/deletion, scrolling, alternate-screen escape sequences, per-cell SGR attributes, ANSI/256/RGB foreground and background colors, current-background erase behavior, and two-cell CJK/wide-rune placement, wrapping, overwrite, capture, and redraw |
| Basic multi-pane redraw | Implemented for styled screen-backed pane snapshots with per-line SGR reset and simple ASCII borders; attached-client manual verification covers ANSI, 256-color, RGB, and combined attribute output, but full tmux-style redraw is not complete |
| Basic `capture-pane` | Implemented for `-p`, visible and scrollback grid lines, tmux-style negative `-S`/`-E` ranges and `-S -`, simple pane targets in the probe subset, basic `-N`/`-T` whitespace handling, basic `-J` wrapped-line joining, signed `-L` line numbers, visible-line `-F` flags, basic `-C` visible text escaping, and tmux-compatible `-e` SGR output for ANSI/256/RGB colors and common attributes |
| Basic `clear-history` | Implemented for clearing the target Screen's scrollback grid while leaving its visible screen unchanged; `history_size` and `history_limit` formats are supported |
...
| Full layout rendering | Pane geometry, basic zoom, and basic redraw exist for a small subset, but full layout algorithms, complete floating pane behavior, tmux-style border cells, custom layouts, and pane movement are incomplete. |
| Screen model | A styled visible grid and bounded styled scrollback grid, common CSI/SGR parser, alternate-screen switching without history pollution, current-background erase, tmux-style scroll-on-clear, and CJK/wide-rune placeholder cells exist; combining characters are ignored in this first wide-character pass, while extended underline/overline and hyperlink cells, redraw diffing, and terminal capability negotiation remain incomplete. |
| Full copy and choose modes | Entry commands are recognized for the basic CLI subset and `choose-tree`/`choose-buffer` have attached summary feedback, but full copy-mode, clock-mode rendering, choose-mode UI, command prompts, and interactive mode key behavior are not implemented. |
...
| Full `capture-pane` semantics | Alternate screen selection, mode screen capture, complete `-J`/`-T` behavior across mode grids, extended styles, hyperlinks, all tmux line flags, and complete target resolution are incomplete. |
