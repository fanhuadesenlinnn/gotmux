...
| Basic terminal screen grid | Implemented for common printable output, cursor movement, clear line/screen, insertion/deletion, scrolling, alternate-screen escape sequences, per-cell SGR attributes, ANSI/256/RGB foreground and background colors, and current-background erase behavior |
...
| Screen model | A styled visible grid and bounded styled scrollback grid, common CSI/SGR parser, alternate-screen switching without history pollution, current-background erase, and tmux-style scroll-on-clear exist; extended underline/overline and hyperlink cells, wide-character handling, redraw diffing, and terminal capability negotiation are incomplete. |
