...
| Common window and pane commands | Implemented, including basic targeted `select-window -t`, `select-pane -t`, `kill-pane -t`, `kill-window -t`, `rename-window -t`, and explicit-target `swap-window` |
