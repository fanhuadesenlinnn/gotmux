...
| Full `capture-pane` semantics | History ranges, alternate screen selection, mode screen capture, paste buffers, complete `-J`/`-T` behavior, escape/style output, hyperlinks, all tmux line flags, and complete target resolution are incomplete. |
