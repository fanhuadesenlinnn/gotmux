...
| Basic key bindings | Implemented for `bind-key`, `bind-key -N` notes, `unbind-key`, `list-keys`, basic `list-keys -N` note output, prefix dispatch, root table dispatch for simple keys, and `send-prefix` |
