...
| Common window and pane commands | Implemented, including basic targeted `select-window -t`, `select-pane -t`, `kill-pane -t`, `kill-window -t`, `rename-window -t`, explicit-target `swap-window`, and basic `move-window`/renumber |
