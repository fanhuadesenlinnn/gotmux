...
| Basic environment commands | Implemented for `set-environment`, `show-environment`, `-g`, `-t`, `-u`, `-s`, basic hidden `-h` variables, and basic remove-marker `-r` variables in the probe subset |
